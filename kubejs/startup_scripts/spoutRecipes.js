// CREDIT TO ARCANE ENGINEERING FOR THIS FILE
...
onEvent("create.spout.special", (event) => {
    ...
    let spoutInteraction = (
        ...
        event.addSpoutHandler(
            blockId + "_spout_interaction",
            blockId,
        ...
        */
                if (!requiredAmount) requiredAmount = fluidAmount;
                if (fluid.id == fluidId && fluid.amount >= requiredAmount) {
                    if (!simulate)
                        if (result)
                            block.setBlockState(
                                Block.getBlock(result).defaultBlockState(),
                                2
                            );
                        else
                            block.blockState.randomTick(
                                block.level.minecraftLevel,
                                block.pos,
                                block.level.minecraftLevel.random
                            );
                    return fluidAmount;
                }
                return 0;
    ...
    buddingBlocks.forEach((Block) => {
        spoutInteraction(Block, "kubejs:crystal_fertilizer", 250);
    });
